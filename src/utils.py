...
def validate_ip(ip: str) -> bool:
    ...
    if not match:
        console.print(f"[red]Error:[/red] Invalid IP address format '{ip}'. Expected format: xxx.xxx.xxx.xxx")
        return False
    ...
    for octet in octets:
        if octet > 255 or octet < 0:
            console.print(f"[red]Error:[/red] Invalid IP address '{ip}'. Each octet must be between 0-255.")
            return False
    
    # Check for reserved/invalid ranges
    if octets[0] == 0 or octets[0] == 127 or octets[0] > 223:
        console.print(f"[red]Error:[/red] Invalid IP address '{ip}'. IP appears to be in a reserved range.")
        return False
...
def check_docker() -> bool:
    ...
    try:
        ...
        console.print("[red]Error:[/red] Docker Compose is not installed or not in PATH")
        return False
...
def validate_env(env_file: str) -> bool:
    ...
    required_vars = [
        "DEPLOY_IP", "GRAPHDB_PORT", "CORE_API_PORT", 
        "MODELS_MANAGER_PORT", "VALIDATION_ENGINE_PORT", 
        "SCENARIO_MANAGER_PORT", "REACT_DASHBOARD_PORT"
    ]
    ...
    try:
        ...
        for var in required_vars:
            if f"{var}=" not in content:
                console.print(f"[yellow]Warning:[/yellow] Required variable {var} not found in {env_file}")
...
class DockerComposeManager:
    ...
    def _run_command(self, args: List[str], capture_output: bool = False) -> Optional[str]:
        """
    ...
    def down(self, profile: str, volumes: bool = False, remove_orphans: bool = False) -> None:
        """
