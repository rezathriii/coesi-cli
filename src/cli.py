...
This module provides the main CLI interface for managing COESI Platform deployments.
It replicates the functionality of the original deploy.sh bash script in Python.
"""
...
def dev(ip: Optional[str]) -> None:
    ...
    """
    if ip:
        console.print("[red]Error:[/red] Development profile does not accept IP parameter.")
...
@main.command()
@click.argument('profile', type=click.Choice(['dev', 'prod', 'all']), required=False, default='all')
...
def ip(ip_address: str) -> None:
    ...
    try:
        ...
        console.print(f"Run 'coesi prod' to deploy with new IP")
...
def deploy_environment(profile: str, ip: Optional[str], rebuild: bool = True) -> None:
    ...
    if not validate_env(env_file):
        console.print(f"[red]Error:[/red] Environment file validation failed: {env_file}")
        sys.exit(1)
    ...
    console.print(f"Ports: GraphDB:{ports_info.get('GRAPHDB_PORT', 'N/A')}, "
                 f"Core:{ports_info.get('CORE_API_PORT', 'N/A')}, "
                 f"Models:{ports_info.get('MODELS_MANAGER_PORT', 'N/A')}")
    console.print(f"       Validation:{ports_info.get('VALIDATION_ENGINE_PORT', 'N/A')}, "
...
def restart_services(profile: str) -> None:
    ...
    console.print(f"[bold blue]=== Restarting COESI Platform ({profile}) ===[/bold blue]")
    console.print(f"Deploy IP: {deploy_ip}")
    console.print("")
    ...
    try:
        if profile == "all":
            ...
            try:
                ...
                console.print("[yellow]Warning:[/yellow] Could not run docker system prune")
            
            console.print("[green]All environments cleaned.[/green]")
        else:
            if not force:
                console.print(f"Cleaning {profile} environment...")
                console.print(f"This will remove containers, networks, and volumes for {profile} profile.")
                if not click.confirm("Are you sure?"):
                    console.print("Clean operation cancelled.")
                    return
